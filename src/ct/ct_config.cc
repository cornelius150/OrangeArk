...
void CtConfig::_populate_keyfile_from_data()
{
    ...
    _uKeyFile->set_boolean(_currentGroup, "codebox_auto_resize_w", codeboxAutoResizeW);
    _uKeyFile->set_boolean(_currentGroup, "codebox_auto_resize_h", codeboxAutoResizeH);
...
void CtConfig::_populate_data_from_keyfile()
{
    ...
    // OrangeArk 1.1.2 hotfix 2: 20px also felt too large — settle back on the
    // original 16px. One-time move for installs sitting on the 20px default.
    if (toolbarIconSize == 2 and not _uKeyFile->has_key(_currentGroup, "toolbar_icon_size_v5")) {
        toolbarIconSize = 1;
        _uKeyFile->set_integer(_currentGroup, "toolbar_icon_size_v5", 1);
    }
    ...
    _populate_bool_from_keyfile("codebox_auto_resize_w", &codeboxAutoResizeW);
    _populate_bool_from_keyfile("codebox_auto_resize_h", &codeboxAutoResizeH);
