...
TEMPLATE_METAINFO_XML_FILEPATH = os.path.join(DATA_DIR, "_net.giuspen.orangeark.metainfo.xml")
METAINFO_XML_FILEPATH = os.path.join(DATA_DIR, "net.giuspen.orangeark.metainfo.xml")
DEBIAN_CHANGELOG_PATH = os.path.join(ROOT_DIR, "debian", "changelog")

VERSION = "?"
with open(DEBIAN_CHANGELOG_PATH, "rb") as fd:
    for fileline in fd:
        match = re.search(b"orangeark +\\(([0-9]+\\.[0-9]+\\.[0-9]+)[-+]", fileline)
